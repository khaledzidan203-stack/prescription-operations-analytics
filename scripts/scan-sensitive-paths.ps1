...
foreach ($file in $files) {
    ...
    if (-not $isText -or $relative -in @(
        "scripts\scan-sensitive-paths.ps1",
        "tests\publication-safety\test_publication_safety.py"
    )) {
        continue
    }
    ...
    $rules = [ordered]@{
        ...
        "private-term-a" = "(?i)Wasfaty"
        "private-term-b" = "(?i)Run-X"
        "private-term-c" = "(?i)Pick-up e-RXs"
        "private-term-d" = "(?i)WL e-RXs"
        "private-schema-term" = "(?i)PrescriptionGroup"
    }
...
if ($findings.Count -gt 0) {
    ...
    $findings | Sort-Object Rule, Path -Unique | Format-Table -AutoSize
    exit 1
}
